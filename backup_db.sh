...
if [ -s "${TABLESPACE}" ]; then
  ...
  uv run --frozen --no-sync aws s3 cp "/tmp/${APP_NAME}.db" "${S3_BACKUP_PATH}" --only-show-errors
else
  log "restoring ${S3_RESTORE_PATH} to ${TABLESPACE}..."
  uv run --frozen --no-sync aws s3 cp "${S3_RESTORE_PATH}" "${TABLESPACE}" --only-show-errors
fi
