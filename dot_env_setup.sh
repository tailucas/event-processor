...
if [ -z "${GITHUB_ENV:-}" ]; then
  ...
  uv run --frozen --no-sync cred_tool "ENV.${PROJECT_NAME}" "${PROJECT_NAME}" build | jq -r '. | to_entries[] | [.key,.value] | @tsv' | tr '\t' '=' | sed 's/=\(.*\)/="\1"/' > "${DOT_ENV_FILE_PATH}"
else
  echo "Generating "${DOT_ENV_FILE_PATH}" from application definition due to CI ${GITHUB_ENV:-}..."
